...
class _ProviderDetailsViewState extends State<_ProviderDetailsView> {
  static const int _labDaysToShow = 30;
  ...
  bool get canProceed {
    ...
    if (widget.provider.providerType == "Nurse") {
      if (selectedService == "Hourly Rate") {
        // ✅ Hourly Rate فقط يحتاج hours
        return selectedDate != null &&
            selectedTime != null &&
            countSelectedhours != null &&
            countSelectedhours! > 0;
      }
      // ✅ Home Visit يحتاج بس date + time
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: isDisabled
                  ? _borderColor
                  : (isSelected ? Colors.teal.withOpacity(0.1) : _cardBg),
              borderRadius: BorderRadius.circular(15),
              border: Border.all(
                color: isSelected ? Colors.teal : _borderColor,
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontWeight: isToday
                            ...
                            : FontWeight.normal,
                        fontFamily: 'Agency',
                        color: isToday ? Colors.teal : _primaryText,
                      ),
                    ),
                    const Spacer(),
                    isOpen
  ...
  List<DateTime> _generateAvailableDays(WorkingDays workingDays) {
    ...
    // Collect the next available lab days, not just the next calendar week.
    while (availableDays.length < _labDaysToShow && daysChecked < 60) {
      final date = DateTime(
        now.year,
        now.month,
        now.day,
      ).add(Duration(days: daysChecked));
      ...
      children: [
        ...
        SafeArea(
          child: IconButton(
            ...
            icon: Icon(Icons.arrow_back_ios_new, color: _cardBg),
              ...
              if (amount != null) ...[
                Text(
                  ...
                  style: TextStyle(
                    fontSize: 12,
                    fontWeight: FontWeight.bold,
                    color: isSelected ? Colors.green : _secondaryText,
                    fontFamily: 'Agency',
                  ),
                ),
              ] else if (isSelected) ...[
